...
void filecount(char filename[] , int letterlist[], int *ccount, int *lettercount);

void countper(int letterlist[] , int lettercount);
void countfreq(int letterlist[]);
...
int main(){
   ...
   int ccount = 0, lettercount = 0;  //count variables
   ...
   filecount(filename, letterlist, &ccount, &lettercount);

   printf("Summary for %s\n", filename); //output
   printf("There were %d characters and %d letters", ccount, lettercount);
   printf("Letter counts:\n");

   countfreq(letterlist);
   countper(letterlist, lettercount);
